...
Run a focused investigation and produce a concise, evidence-backed report. This is an ability skill, not a stage: use it inside `stage-idea-refine`, `stage-goal-clarify`, or normal Codex mode when the question is too complex for a quick lookup.
...
- Keep noise out of the main context: follow the global **Background-First Prefetch** protocol when delegating high-noise branches to `lab-explore` / `lab-research`, then synthesize only durable facts.

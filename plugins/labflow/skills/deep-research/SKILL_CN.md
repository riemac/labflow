...
- 高噪音分支：需委派 `lab-explore` / `lab-research` 吸收检索噪音，但主 agent 必须回收并核验关键结论。
...
- 不用二手资料替代官方文档、源码或论文。
- 简单代码定位不要用本 skill，使用 `codebase-research`。
- 简单 API 语法问题不要用本 skill，使用 `find-docs` / `external-research`。

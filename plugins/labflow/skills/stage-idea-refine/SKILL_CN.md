...
`problem_statement` 是所有 stage 共享的问题锚点：当前我们认为这个 stage 围绕什么问题、目标或研究问题展开。`problem_clarity` 表达这个锚点有多可靠：

- `unknown`：agent 还不能可靠复述问题。
- `fuzzy`：大方向存在，但对象、边界、动机、成功信号或约束还不稳。
- `framed`：问题已经能被清楚复述，用户大体认可，但仍可能因证据、反例或方向切换而改写。
...
`idea_state` 是自由文本，用来描述围绕 `problem_statement` 已形成的当前路线、理解、假设或风险。它比 `exit_readiness` 更有科研语义，也更适合给用户和 agent 回顾。
...
`UserPromptSubmit` hook 会持续注入当前 `problem_statement` / `problem_clarity` / `exit_readiness` / `idea_state`，提醒 agent 仍处在该 stage。hook 不负责理解科研语义，语义判断由 agent 完成。若问题锚点为空或仍是 `unknown` / `fuzzy`，应优先澄清问题，不要过早收敛技术路线。

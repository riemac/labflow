...
- 如果 stage 是因讨论文件名、hook、命令本身而误触，agent 应主动说明并用独立 `$stage-cancel` 退出。
...
当 `exit_readiness=ready_to_pass` 时，agent 优先提醒用户发送 `$stage-pass`。只有在用户明确要求 agent 代为退出 stage 时，才由 agent 自己输出独立 `$stage-pass` 行。
...
abilities 暂不在本文件中设计。需要 deep research、paper reading、codebase research 等能力时，按当前任务和用户显式需求调用已有 skill。

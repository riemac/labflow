...
├── _progress.md           ← 工程续接快照（每次对话必读，覆写，不限长度）
...
```bash
# 覆写工程续接快照；_progress.md 不是历史日志
obsidian vault="Research" create name="_progress" content="<完整当前续接文档>" overwrite silent

# 覆写研究语境快照；保持短小，避免追加成历史记录
obsidian vault="Research" create name="_context" content="<完整当前语境快照>" overwrite silent
```

...
科研 R&D 项目高度非线性和易变，很难 `PRO -> SPEC -> 实现` 的软件工程 agent 流水线一次成功，会随效果与想法不断迭代。Agent 因上下文窗口限制，缺乏全局视野，难以判定当前所处 stage。但用户具备持久全局感知，知道当前是何 stage：讨论想法、澄清目标、任务规划，还是编码实现。你负责识别并尊重当前 stage，在该阶段内遵从各 stage 预定义的行为模式。

SDD 是一层轻量协作约束，不是重型流程编排器。它减少误解和返工，而不是制造额外仪式感。因而足够轻量友好、易模块化、扩展性强。
...
- `Abilities`：阶段中可调用的能力，如 deep-research、deep-interview；能力可被不同 stage 复用，不绑定死。
- `Hooks`：维护轻量状态与提醒，如进入阶段、注入上下文、通过或取消阶段。

stage 本身也由 skill 构成，而 `Ability` 是同层级别的 skill，但能力更聚焦于阶段内的特定行为模式。一个 stage 可以调用多个 ability 来完成阶段目标。

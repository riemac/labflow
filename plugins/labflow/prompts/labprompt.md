...
- **Background-First Prefetch**：进入调研后先找入口和候选模块；如果当前 Codex 环境支持且用户允许委派，把边界明确的代码调研异步交给 `lab-explore`，不要等主线阻塞后才派
- **主会话继续推进**：subagent 运行时，主会话选择性读取不重叠的关键文件；先做路径约束的本地语义检索 / 宽搜索，再补读入口、目标函数、相邻模板和配置注册点
- **依赖屏障再回收**：只有下一步依赖 subagent 结果且没有其他有效工作时，才使用 Codex 原生等待/完成通知；不得 shell sleep、反复轮询或重复调研
- **关键事实必须核验**：subagent 返回内容是高信号预取；凡是写入最终提示词的路径、函数名和当前逻辑，都要由主会话回到代码核验

...
Subagent 主要承担 read-heavy、检索密集或其他上下文噪音高的工作。委派的目的是让
检索噪音留在 worker 上下文中，主 agent 则保留用户目标、编排权、最终决策和面向
用户的综合。

## 后台优先预取

把委派当作预取，而不是阻塞式交接。先判断一个边界明确的任务是否能实质推进当前
工作；一旦决定委派，只要运行时支持，就默认后台启动，即使后续步骤很快会依赖该
结果。主 agent 随即继续有意义且不与 worker 重叠的工作。推进到依赖屏障后，再使用
运行时原生等待机制，或让出当前 turn 等待完成通知。

```mermaid
flowchart TD
    A[预判高噪音或上下文密集工作] --> B[在后台启动边界明确的 worker]
    B --> C[继续有意义且不重叠的工作]
    C --> D{已到依赖屏障?}
    D -- 否 --> C
    D -- 是 --> E{worker 结果已返回?}
    E -- 是 --> F[核验关键事实并整合]
    E -- 否 --> G[使用原生等待或让出 turn 等待完成通知]
    G --> F
```

等待时不得用 shell sleep、轮询任务状态，也不得重复执行 worker 的任务。

## 所有权与整合

给每个 worker 明确的范围、预期输出和所有权边界。默认委派 read-heavy 工作。只有
明确分配的辅助产物或与主 agent 完全分离的写入集合才可交给 worker；主 agent 不得
同时编辑相同文件。worker 结果是高信号预取，但凡直接驱动编辑、科研结论或最终回答
的精确事实仍需核验。不要委派最终决策或面向用户的综合。

## 持续性委派

后续问题、工作线或证据链只要仍有实质关联、共享重要上下文，或与先前工作存在尚不
明确但合理的联系，就续接同一 subagent，而不是重新 spawn。结果不完整或有误时，
只要旧上下文仍有用，就向原 subagent 补充纠偏信息。只有范围明确变化、需要独立
复核，或旧上下文噪音过多、已无用时才新建。领域 skill 可以规定更严格的续接边界。

## 平台说明

Codex 的 `spawn_agent` 会异步启动 subagent。派出后继续不重叠工作，只在依赖屏障
调用 `wait_agent`。相关工作在 subagent 仍 open 时使用 `send_input`；已关闭时先
`resume_agent`，再 `send_input`。不要用 shell sleep 或轮询替代这些原生生命周期
工具。

...
test("autopilot uses one maintained Goal and native operational state", async () => {
  ...
  assert.match(source, /<activation_contract>/)
  assert.match(source, /<flipped_preflight>/)
  assert.match(source, /Treat the user's first statement as motivation, symptoms, direction, or a desired change/)
  assert.match(source, /<maximum_useful_parallelism>/)
  assert.match(source, /maximum useful parallelism, not maximum process count/i)
  ...
  assert.match(chinese, /<flipped_preflight>/)
  assert.match(chinese, /最大有效并行/)
  assert.match(chinese, /```mermaid/)

...
Model provider channels and secrets live in `/home/hac/labflow/provider/` as the repository-wide Single Source of Truth; OpenCode consumes the self-contained `@labflow/provider` module directly in `opencode/scripts/config.mjs` and injects credentials dynamically in-memory.
...
├── scripts/
│   ├── config.mjs            # imports @labflow/provider, applies defaults, secure fetch hook
...
├── config/                   # tracked defaults, plugins, imagegen routes (no fake symlinks)
...
│   ├── compaction.md         # English runtime prompt; compaction_CN.md is review-only
...
`literature-forensics` is a cross-platform ability skill backed by a shared-format local Python CLI and platform-specific `literature-worker` subagents. The primary agent remains the
...
Compaction uses `agents/compaction.md` as its sole runtime prompt; `compaction_CN.md` is an unregistered Chinese review companion. The plugin replaces the native request template through `experimental.session.compacting` and restores the latest successful prior summary. Its immediately following `experimental.chat.messages.transform` also supplies the retained recent context to the disposable compaction input, without changing native replay or persisted history. Keep the pending snapshot session-scoped and short-lived; history-read and boundary errors stop compaction rather than falling back. Only compaction requests receive a 64,000-token output budget, capped by a valid model output limit; native OpenAI OAuth omits this unsupported parameter. Keep the English/Chinese semantics aligned, preserve model selection in `config/defaults.yaml`, and run `tests/compaction.test.mjs` plus the integration suite after changes. Verify actual retained-tail compaction separately from mocked hooks and small sessions that never split a tail.
...
The primary agent remains the default implementer and may delegate bounded implementation, tests, reviews, or short probes to native `general` when the expected benefit justifies coordination. Delegation preserves the current stage and authorization boundaries; the primary owns integration and final verification. Labflow leaves `general` behavior native and preserves user configuration overrides.

Read-only local or external exploration uses the hidden `explore-worker` with explicit `fast`, `normal`, or `deep` scope profiles; `normal` is the default. The worker cannot edit or delegate. Its prompt and permissions live in `agents/explore-worker.md`; the portable default model and reasoning options live in `config/defaults.yaml`, where built-in `explore` remains disabled. The plugin merges user agent config last so users may replace the provider/model/options without forking the worker behavior.
...
- `labflow-develop` is the nonlinear research-dialogue agent for natural intent discovery, evidence-guided idea refinement, method design, mathematical explanation, recoverable decision-tree/record checkpoints, and non-executable design scaffolds. Switch into it (Tab) for the develop stage.
...
- No hidden state-persistence plugin: substantial Develop sessions may explicitly checkpoint paired Markdown artifacts under the project's docs, while the user remains in control of their location and canonical status.

## De-Codex mapping

When adapting a shared skill from `codex/skills/` into
`opencode/skills/`, rewrite these Codex-isms:

| Codex | OpenCode |
|---|---|
| `request_user_input` | `question` tool |
| native `explorer` / bounded `default` retrieval subagents | unified `explore-worker` subagent |
...
Codex no longer ships Labflow stages or primary-agent prompt injection. OpenCode keeps its own agent-switching behavior. Its `pdf-read` skill uses the PDF reader evidence interfaces, with CLI or native attachment handling as fallbacks.
...
2. Edit under `opencode/`. Research contracts and dossier helpers are shared with
   `codex/skills/` where useful; keep platform orchestration separate
   and preserve shared dossier formats.

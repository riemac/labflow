...
`deep-research` 是一个 ability skill。它适合在 labflow develop 阶段或任意普通 agent 对话中被调用，用来处理“短回答不够、必须查证和综合”的复杂问题。
...
- 高噪音分支：需委派内置 `explore`（本地代码）/ `scout`（外部文档）subagent 吸收检索噪音，但主 agent 必须回收并核验关键结论。

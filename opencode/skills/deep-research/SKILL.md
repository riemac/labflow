...
- Keep noise out of the main context: follow the global exploration profile and **Background-First Prefetch** contract when delegating bounded high-noise branches to `explore-worker`, then synthesize only durable facts.
...
2. **Collect evidence**
   ...
   - For broad/noisy branches: delegate bounded read-heavy work to `explore-worker` only when it protects the main context; treat returned evidence as high-signal prefetch, but verify any claim that drives the conclusion.

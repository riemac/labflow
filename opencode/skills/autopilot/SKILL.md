...
Before starting, inspect the project and teach back one concise interpretation of the intended outcome, success and failure boundaries, fixed decisions, non-goals, human-only gates, resource envelope, and forbidden side effects. Ask one grouped question only when a missing high-impact boundary would make autonomous execution unsafe or likely to pursue the wrong target.
...
## Native State And Documentation
...
## Parallel Work And PTYs

Use parallelism only when independent lanes materially reduce time or anchoring bias. Direct execution is preferred for a clear local route. Every parallel lane needs a distinct question or isolated output boundary, and the primary agent retains integration responsibility.

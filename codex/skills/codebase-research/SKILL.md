...
Use the native `explorer` for unfamiliar subsystems, cross-module chains, or multiple candidates likely to consume the main context. Give each independent worker a bounded read-only question, spawn asynchronously, and continue non-overlapping work. Collect native completion messages and wait only when the next step depends on the result. Reuse a worker for the same evidence chain using the host's available follow-up tools; use `fork_turns="none"` with an explicit assignment and respect configured subagent model/effort defaults. Omit runtime model and effort overrides unless the user requests them.

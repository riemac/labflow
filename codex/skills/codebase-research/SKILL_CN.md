...
陌生子系统、跨模块链路或多个候选可能占用大量主上下文时，使用原生 `explorer`。每个独立 worker 获得有界只读问题，异步启动后主代理继续不重叠工作；通过原生完成消息收回结果，只在下一步依赖结果时等待。同一证据链用宿主实际可用的 follow-up 工具续接原 worker，通过 `fork_turns="none"` 显式传入任务，遵守用户配置的子代理模型与推理强度；除非用户要求，不传运行时覆盖参数。

- 优先给聚焦范围；但问题本身跨模块时，可以让一个 subagent 覆盖多个相关模块。

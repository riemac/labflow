...
codex_bin="${LABFLOW_CODEX_BIN:-codex}"
...
if ! "$codex_bin" plugin marketplace remove "$marketplace_id"; then
...
if ! "$codex_bin" plugin remove "$plugin_selector"; then
  printf 'Plugin `%s` was not installed; continuing with add.\n' "$plugin_selector" >&2
fi

"$codex_bin" plugin add "$plugin_selector"

# The runtime loads regular TOML files; preserve user edits via the hash ledger.
codex_home="${CODEX_HOME:-${HOME}/.codex}"
cache_root="${codex_home}/plugins/cache/${marketplace_id}/${plugin_name}"
if [[ -d "$cache_root" ]]; then
  ...
  # Resolve the versioned cache after reinstall so reviewer skill toggles point
  # at the actual files loaded by this session.
  python3 "$script_dir/install_workers.py" \
    --source-dir "$marketplace_root/codex/agents" \
    --target-dir "$codex_home/agents" \
...
printf 'Reload complete. Start a new Codex session to observe refreshed skills and custom workers.\n'

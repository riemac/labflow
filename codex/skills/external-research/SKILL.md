...
For substantial independent external evidence lanes, use a native `default` subagent with a bounded question, official-source scope, stopping conditions, and return contract. Use `literature-worker` through `literature-forensics` for scholarly prior-art investigations; native `explorer` is reserved for local codebase questions.

Spawn asynchronously, continue non-overlapping work, collect native completion messages, and wait only at a dependency barrier. Reuse the same worker for the same evidence chain using the host's current follow-up tools. In the current App these are `send_message` while running and `followup_task` after completion. Use `fork_turns="none"` with a complete assignment and respect configured subagent model/effort defaults. Omit runtime overrides unless the user requests them. Keep retrieval read-only unless an exact disjoint support artifact is assigned; the primary owns synthesis and verifies decisive sources.

...
Investigate prior art as a research lead, not as a search-result summarizer. Turn a research hunch into bounded questions, search several evidence paths, inspect the primary papers that matter, and leave a dossier that a researcher wants to read and can independently audit.
...
Infer the research language from the user's language unless they explicitly choose another. Persist it as `language` in `.research/brief.md`, and pass it in every worker assignment. Human-facing research artifacts and chat synthesis use that language; official paper titles, technical identifiers, and direct quotes may remain in their source language.

The English skill is the executable primary protocol. `SKILL_CN.md` is its maintained Chinese counterpart; update both when behavior changes.
...
Workers improve recall and locate evidence. They do not own novelty judgments or write human-facing synthesis.

## 1. Frame The Research Question

Read `references/research-frame.md`. Establish only information that changes the investigation:
...
Agent audit material belongs under `.research/`. Initialize or validate the structure with the standard-library helper:
...
When invoked outside this skill directory, use the absolute path to `scripts/research.py`. The helper only manages structure; it never writes research conclusions.
...
Prefer `--jsonl`, `--ids-only`, stdin, `jq`, and `rg` pipelines when they reduce context use. Do not expose routine CLI mechanics to the user.

## 4. Delegate Progressive Worker Assignments

Use the dedicated `literature-worker` custom agent installed from `agents/literature-worker.toml` by the reload helper. Spawn independent bounded lanes asynchronously with `fork_turns="none"`, supplying the complete assignment and source paths. Continue non-overlapping lead work while workers run; collect results through native completion messages and wait only at a dependency barrier. Use the model and reasoning effort from the user-managed role/default configuration; do not pass runtime overrides unless the user requests an exception.

Use the lifecycle tools actually exposed by the host: in the current App, `spawn_agent` starts a worker, `send_message` steers a running worker, and `followup_task` continues the same worker after it becomes idle. Older clients may expose `send_input`/`resume_agent` instead. Do not use user-owned sidebar tasks as a substitute for subagents. If the custom role is unavailable, load its tracked TOML instructions into a native `worker` assignment and report the registration gap; this fallback does not validate custom-role discovery.

Record the returned worker handle and lane in `.research/state/workers.json`. A stored handle is host-specific routing metadata, not a guarantee of cross-session resumption. If the host cannot resume it, create a replacement worker from the same brief and assigned audit artifacts, preserving the dossier and recording the replacement.

Use `literature-worker` for one bounded lane and one profile per assignment. Read `references/worker-contract.md`. Every prompt should explicitly provide:
...
Use `pdf-read` for page maps, text evidence, figures, captions, and crops. Worker reading does not replace lead verification for exact matches, strongest analogues, counterevidence, or paper-facing claims.

## 6. Synthesize Human Reports

Workers write only `.research/audit/lanes/` and assigned `.research/audit/papers/` artifacts. The coordinator synthesizes:
...
Do not create a visible paper-card directory. Integrate important paper-level analysis into the relevant topic report; keep audit cards hidden.
...
Do not foreground artifact paths, provider failures, task IDs, worker budgets, or search-process narration. Coverage limitations belong in the research answer only when they affect confidence.

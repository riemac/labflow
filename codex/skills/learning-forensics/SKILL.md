...
Use Codex's native background-agent mechanism. Prefer the custom `learning-worker` installed from `codex/agents/learning-worker.toml`. Respect its user-configured model and effort; omit runtime overrides unless the user explicitly requests them. When it is unavailable, use the built-in `worker`; its spawn prompt must load `learning-forensics`, name the phase and lens, include the complete assignment schema, and restate the blind-input, hidden-write, bounded-probe, and return contracts. Spawn blind workers with `fork_turns="none"` and the complete sealed assignment; default full-history forks would disclose coordinator hypotheses. If the host cannot start without inherited history, disclose the limitation and do not label the round blind. Spawn asynchronously, continue non-overlapping work, reuse the same agent for the same lens and evidence chain, and wait only at a real dependency barrier. Use the host's available lifecycle tools: current App `send_message` for a running agent and `followup_task` for an idle one, or older `send_input`/`resume_agent` where exposed. Use the native `explorer` for local code retrieval and a bounded native `default` agent for external retrieval. Handles are host-specific routing metadata; if unavailable after a restart, recover a replacement from the same assigned artifacts and record the replacement. Never recover a new blind lane from diagnostic audit notes; use its sealed facts only.

Record every returned thread handle in `.learning/state/workers.json`. Each blind worker returns at most three falsifiable hypotheses, including mechanism, supporting evidence, contradictory evidence, missing evidence, and a decisive test with predictions under both outcomes. It also writes only its assigned hidden lane artifact.
...
- **primary-led convergence:** the primary owns all experiments and decisions, while `learning-worker` remains available for an independent causal challenge and `explorer` may retrieve bounded read-heavy code, artifact, documentation, or source evidence;

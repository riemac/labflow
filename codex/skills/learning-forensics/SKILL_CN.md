...
Codex 优先使用通过 `codex/agents/learning-worker.toml` 安装的自定义子代理。遵守用户配置的角色模型与推理强度，除非用户明确要求，不传运行时覆盖参数。该角色尚未安装时，内置 `worker` 的启动提示需要加载 `learning-forensics`，写明阶段与视角，携带完整委派格式，并重申盲审输入、隐藏写入、有界探针和返回合同。盲审必须以 `fork_turns="none"` 和完整 sealed assignment 启动；默认继承全部历史会泄漏主代理假设。宿主无法隔离继承历史时，应说明限制，不能声称该轮为盲审。异步派出后继续互不重叠的主线工作，只在依赖边界等待。当前 App 对运行中的 worker 用 `send_message`，对空闲 worker 用 `followup_task`；旧客户端按实际可用的 `send_input`/`resume_agent` 操作。本地源码检索使用原生 `explorer`，外部检索使用有界原生 `default`。返回的 worker handle 写入 `.learning/state/workers.json`。句柄是宿主相关的路由元数据；重启后不可用时，从同一已分配 artifact 恢复替代 worker 并记录替换。新的盲审 lane 只能从封存事实恢复，不能读取已有诊断审计材料。
...
- **primary-led convergence：** 主代理拥有全部实验和决定；`learning-worker` 仍可提供独立因果挑战，`explorer` 可承担源码、artifact、文档和外部来源等有界 read-heavy 检索；
- **re-expansion：** 新的矛盾证据打开另一条因果分支时，在同一 dossier 内增加新相关且不重复的 lanes，不重启 dossier，也不机械重跑原始首轮。

这些是随证据变化的调查阶段，不是用户启动时选择的固定模式，也不是只能升级的单向状态机。证据收缩后立即降级；只有新区别会改变决定时才重新扩展。`learning-worker` 负责因果分析，`explorer` 负责检索支持，两者不互相替代。
